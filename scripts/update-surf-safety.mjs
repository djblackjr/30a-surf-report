...
Search for TODAY's rip current risk category and surf height range for the South Walton / 30A coast (Walton County, FL — Dune Allen Beach, Blue Mountain Beach, Grayton Beach, Seagrove Beach, Inlet Beach), from the NWS Surf Zone Forecast (issued by NWS Tallahassee, forecast.weather.gov). Don't bother with Surfline — its site sits behind a Cloudflare JS challenge that blocks both plain fetches and your web search tool, so it almost never actually yields a genuinely current reading; it's not worth the search budget.
